...
class ATMType extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    return (
      <>
        <select name="atmType" onChange={this.handleChange} value={(this.props.value) ? this.props.value : null}>
...
export class ClientSelfservice extends React.Component {
  constructor(props) {
    super(props);
    this.state = {modalError: null, isChange:false};
    ...
    if (text_error) this.setState({modalError:(<Modal title={"Ошибка"} onOK={()=>{this.setState({modalError:null});}}>
      ...
      let client = {
        selfservice: {
          ...
          atmtype: this.state.client.atmType,
    ...
    return (
      <>
        <table>
          ...
          <tr>
            <td style={{['textAlign']:'right'}}>Тип банкомата:</td>
            <td>
              <ATMType onChange={this.handleInputChange} value={client.atmType}/>
            </td>
          </tr>
          <tr>
